    ...
    def create_backup(self, compression_level=9) -> str:
        ...
        try:
            ...
            cmd = [
                ...
                "-v",  # Verbose output
                "-Z",
                str(compression_level),  # Compression level
            ...
            self.details_logger.info(
                f"Backup configuration: compression_level={compression_level}"
            )

            # Execute pg_dump
            start_time = datetime.now()
            result = subprocess.run(cmd, env=backup_env, capture_output=True, text=True)
    ...
    def verify_backup(self, backup_path: str) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                self.details_logger.info(
                    f"Verified backup size: {size / 1024 / 1024:.2f} MB"
                )
                return True
            else:
...
def main():
    ...
    try:
        ...
        if backup.verify_backup(backup_path):
            ...
            if (
                input("Clean up backups older than 30 days? (yes/no): ").lower()
                == "yes"
            ):
