...
class DrugIngredientLinker:
    ...
    def create_gemini_prompt(
        self, drug_entry: Dict, known_ingredients: List[str]
    ) -> str:
        """Create an improved prompt for Gemini API to analyze drug ingredients."""
        ...
        return f'''You are a pharmaceutical database expert. Analyze this drug's active ingredients and map them to known ingredients. Be thorough in checking different forms and variations.
    ...
    Mapping Rules:
    1. Consider pharmaceutical variations:
    - Salt forms (e.g., "brimonidine tartrate" matches "Brimonidine")
    - Common alternative names (e.g., "ASA" matches "Acetylsalicylic acid")
    - Chemical variants (e.g., "glyceryl trinitrate" matches "Glyceryl trinitrate")
    2. Split compound ingredients at +, /, and, &
    3. Remove dosage information and common suffixes
    4. Set confidence scores:
    - 0.95+ for exact matches
    - 0.85-0.94 for matches with known variations
    - 0.80-0.84 for less certain matches
    5. Match based on the main active compound
    6. Consider case-insensitive matching

    Example response:
    {{
        "mappings": [
            {{
                "original": "brimonidine tartrate",
                "matched_ingredient": "Brimonidine",
                "confidence": 0.9,
                "notes": "Matched base compound (removing salt form)"
            }}
        ]
    }}
    ...
    3. Be thorough in checking variations
    4. Consider pharmaceutical naming conventions'''

    def clean_ingredient_text(self, text: str) -> str:
        """Enhanced cleaning of ingredient text for better matching."""
        if not text:
            return ""
        ...
        # Remove common pharmaceutical terms that don't affect matching
        terms_to_remove = [
            r"\d+\s*mg/?ml?",  # Remove dosage
            r"\d+\s*%",  # Remove percentage
            r"injection",
            r"solution",
            r"tablet",
            r"capsule",
    ...
    def validate_mapping(self, mapping: Dict, known_ingredients: List[str]) -> bool:
        """Enhanced validation of ingredient mappings."""
        if not all(
            key in mapping
            for key in ["original", "matched_ingredient", "confidence", "notes"]
        ):
            return False

        # Case-insensitive check for ingredient in known list
        if not any(
            i.lower() == mapping["matched_ingredient"].lower()
        ...
        if not isinstance(mapping["confidence"], (int, float)):
            return False
        if mapping["confidence"] < 0.8 or mapping["confidence"] > 1.0:
            return False
    ...
    def get_unlinked_drugs(
        self, conn, sample_size: int = None, sample_groups: bool = True
    ) -> List[Dict]:
        """
    ...
    def process_batch(
        self, conn, batch: List[Dict], known_ingredients: List[str]
        ...
        for drug in batch:
            try:
                ...
                try:
                    ...
                    self.logger.debug(
                        f"Cleaned API response for {drug['tradename']}: {cleaned_response}"
                    )
                ...
                if not isinstance(analysis, dict) or "mappings" not in analysis:
                    self.logger.error(
                        f"Invalid response structure for drug {drug['tradename']}"
                    )
                    failed_mappings.append(
                        {
                            "drug_id": drug["drug_id"],
                ...
                # Validate mappings using the new validate_mapping method
                valid_mappings = []
                for mapping in analysis["mappings"]:
                    if self.validate_mapping(mapping, known_ingredients):
                        mapping["drug_id"] = drug["drug_id"]
                        mapping["tradename"] = drug["tradename"]
                        valid_mappings.append(mapping)
                    else:
                        self.logger.debug(
                            f"Invalid mapping for {drug['tradename']}: {mapping}"
                        )
        ...
        try:
            with conn.cursor() as cur:
                ...
                for mapping in mappings:
                    try:
                        ingredient_id = ingredient_ids.get(
                            mapping["matched_ingredient"]
                        )
                        ...
                        self.logger.error(
                            f"Error creating link for {mapping['tradename']}: {e}"
                        )
                        errors += 1
                        continue
    ...
    def generate_mapping_report(
        self, mappings: List[Dict], failed_mappings: List[Dict] = None
    ) -> str:
        ...
        try:
            ...
            try:
                ...
                for i in range(0, total_drugs, self.batch_size):
                    ...
                    total_batches = (
                        total_drugs + self.batch_size - 1
                    ) // self.batch_size

                    self.logger.info(f"Processing batch {batch_num}/{total_batches}")

                    errors, mappings = self.process_batch(
                        conn, batch, known_ingredients
                    )
                    total_errors += errors
                ...
                if (
                    input("\nCreate these ingredient links? (yes/no): ").lower()
                    == "yes"
                ):
...
def main():
    ...
    parser.add_argument(
        "--sample", type=int, help="Number of drugs to sample for testing"
    )
    parser.add_argument(
    ...
    try:
        ...
        try:
            ...
            unlinked_drugs = linker.get_unlinked_drugs(
                conn, sample_size=args.sample, sample_groups=not args.random_sample
            )
            total_drugs = len(unlinked_drugs)
            ...
            for i in range(0, total_drugs, args.batch_size):
                ...
                errors, mappings, failed_mappings = linker.process_batch(
                    conn, batch, known_ingredients
                )
                total_errors += errors
                all_mappings.extend(mappings)
